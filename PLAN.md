...
| **M0** 接缝验证 | ✅ **通过** |
| **M1** 接管一台机 | ✅ **通过** |
| **M2** 接管两台 | ⏳ **进行中** —— 自研批次全部做完，剩**退出条件**与**观测**两件 |
| **M3** 对拍达标 | ⏳ 未开工 |
| **M4** 发布 | ⏳ 未开工 |
...
⏳ **未落地** —— 观测三件套还剩 **2/3**（Prometheus 指标、Runtime 实时 stats，全仓
`prometheus` 零命中）· Runtime 增量通道与临时覆盖层 · PROXY protocol 的「发」那半边 ·
被动熔断（`passive_fail`）。
...
1. **观测三件套还剩 2/3** —— Prometheus 指标、Runtime 实时 stats。
   ⚠ **一格里装三件事，最容易被读成一件**；这一格是 **1/3，不是 ✅**。
2. **退出条件本身** —— 两台服务器全部由枢衡承载。

⏳ 另外欠着：**Runtime 增量通道 + 临时覆盖层** · PROXY protocol 的「发」那半边（至今没有任何 DSL 面）。
...
| ⏳ ★ **门禁在 GitHub Actions 上间歇性红，本机从来不红**：ACME 场景的第二代等不到 `9443`，而 `run2.log` 末尾是一串 `127.0.0.1:80 is in use, will try again` | ⛔ **那串重试不是根因。** 反证：整趟占住 `127.0.0.1:80` 再跑，场景照样 41 ✓ / 0 ✗。机制也对得上——pingora 每个 `Service` 在**自己的 runtime** 上 bind（`Server::run_service` 逐个 spawn），`:80` 卡着不会拖住别的监听器；而 `bind_tcp` 重试 30 次 × 1 秒、`wait_port` 20 秒就放弃 ⇒ **日志停在那几行，是观察者先超时的产物，不是证据**。⏳ **真因仍然未知**：那一刻 `9443` 为什么没起来，现有日志一个字都答不出。⇒ 两个 ACME 场景的失败路径上已装好取证：谁占着哪些端口、每个 socket 的**全部**持有者 pid（继承走的 fd 就长这样）、第二代是死是活（连退出码）、以及第二代启动**之前** `:80` 的快照。取证只在已经要红的路径上跑（零假红），它自己由 [`tests/acme/self-check.sh`](tests/acme/self-check.sh) 在 lint 那一格两个方向各钉一条 |
...
| **D26** | **没有站点匹配的那条请求（421，G63）记不进访问日志。** `log` 是站点块内的指令，而 `outcome=no_site_match` 的请求按定义不属于任何站点。要拍的不是一个默认值，而是配置面往哪儿长：① 全局块加一条 `log`（那就要回答「站点自己写了时是覆盖还是合并」）；② 站点级的显式默认；③ 就这样 —— 421 只留 journal 里那行 `debug`。★ 不紧迫的理由是它答不到观测这一批要答的问题上：「线上有人报慢 / 502」那些请求全都已经命中了站点；缺的这一格是「谁在拿奇怪的 Host 打我」，属于安全面。 | M2 收尾前 |
| **D29** | **自动 HTTPS 的重定向端口写死 `:80`。** `synthesize_http_redirect`（`crates/fulcrum-config/src/compile.rs`）给每个自动 HTTPS 站点合成的 308 站点，端口是个字面量，配置面上没有任何一处说得出它。⚠ **它不只是个默认值**：HTTP-01（RFC 8555 §8.3）规定 CA 只连 80 端口，所以「让它可配」与「HTTP-01 还能用」是有张力的 —— 那条挑战正是靠这个站点才有落脚点（G54 把 HTTP-01 定为备）。两条候选：① 全局块加一条 `auto_http_redirect_port`，并写明改了它就等于关掉 HTTP-01；② 维持写死（现状），只在文档里说清楚。★ 顺带的后果已经处理掉了：门禁里七个场景因此隐式共用 `127.0.0.1:80`，这件事现在写在 AGENTS.md 的端口表里，**且有意不加「必须空着」的检查**（占着它已实测无害）。★ 不紧迫：`:80` 是所有 CA 的事实默认，改它只服务于非标准部署。 | M4 发布前 |
| **D9** | **版本与兼容性策略**：何时算 breaking change，DSL 与结构化配置各自的稳定性承诺。 | M4 |
