...
| **D30** | **`status_class` 的第 6 个值。** `fulcrum_requests_total{status_class}` 今天是六个值：`1xx` `2xx` `3xx` `4xx` `5xx` **`none`**，`status == 0`（一个响应头都没写出去 —— 契约原话：那不是「未知」，是**什么都没发生**）归 `none`，1–99 与 600 以上（上游给的编出来的数）也归 `none`。⚠ `status == 0` **可达且会被记进访问日志**：站点匹配上了、执行链跑完了、下游在我们写响应之前断开就是这个形状。三条候选：① **维持现状**（第 6 个值 `none`）；② 这类请求不计进 `requests_total`（⚠ 那样「指标增量 == 访问日志行数」那条一致性门就不成立了）；③ 改用 `0xx`。⛔ 不给 1–99 / 600+ 各开一格 —— 那等于把这一格的**上界交回给别人**，而基数那张表管的正是这件事。**错了的代价**：一个多余的 series 值，改回去只需改一处 `match`。 | M2 收尾前 |
...
| **D31** | **缓存事件的折叠口径与边界。** `fulcrum_cache_events_total{event}` 折成四个值：`hit`（含磁盘后端，⚠ 比 `X-Fulcrum-Cache` 粗 —— `HIT` 与 `HIT-DISK` 同一格）· `miss` · `stale` · `purge`。**边界**：`hit`/`stale` 记在发缓存那一处，`miss` 记在**回源**那一处 —— ⚠ 有意不记在「查缓存没命中」那一处（那里还会拐弯：`only-if-cached` 回 504 根本没回源、防惊群的 follower 等完重查会**命中**，在那里记两种都会被算成 `miss` 而**数字看起来完全正常**）。⇒ `hit + stale + miss` 覆盖的是「查了缓存**并且**已经定下这条响应的内容从哪来」，⚠ **上游连不上的那些请求三格都不在**，所以上游故障时 `hit/(hit+miss)` 会偏高。⚠ **`purge` 与另外三个不在同一个分母里**（前三个的单位是「一条请求」，`purge` 是「一条缓存条目」）⇒ **`sum(cache_events_total)` 不是任何一个有意义的量**；并且 `purge` 清掉 0 条时也记一次 `+0`（让那条 series 尽早存在），与另外三个「发生了才出现」**有意不对称**。要拍的是这套口径本身，以及要不要让「是哪一种命中」也进指标。**错了的代价**：折叠得太粗时「哪种命中」要去看访问日志（它有 `cache` 原值）。 | M3（对拍时会真的去读这些数）|
| **D32** | **「连接」与「TLS」两类指标一个族都没有。** 批 M 落地的九个族覆盖的是请求 · 上游 · 缓存 · 证书 · ACME 签发；**连接类**（并发连接数、连接建立/关闭速率）与 **TLS 类**（握手次数、失败次数、版本/套件分布）**一条都没有**。⚠ ⚠ **先把出处摆正**：G16（§10）的原话只有「观测基线 = Prometheus 指标 + 结构化访问日志 + Runtime 实时 stats」，**它从来没有枚举过要覆盖哪几类**；那句「覆盖连接、请求、上游、TLS、缓存、证书签发」是 `docs/architecture/observability.md` 自己对 G16 的**转述**，没有任何一条 owner 决定背书过它（该句已在批 M 任务 7 里按实际改掉）。⇒ 分歧不在「G16 与 G116 之间」，而在「一句无主的转述与 G116 那张表之间」，**更不是实现漏做** —— G116–G121 拍板时那份指标清单里本来就没有这两类。两条读法：① **那句转述本来就不作数**，G116 那张表是唯一权威 ⇒ 本条直接关掉。代价：**以后没人会再想起这两类**，而它们是 nginx/HAProxy 的 stub_status / `show stat` 里最常被看的几个数。② **是真欠账**，补 `fulcrum_connections_*` 与 `fulcrum_tls_handshakes_*` 一族。⚠ 代价是两类**都踩在批 M 全程在躲的两个形状上**：TLS 那类天然想按 `version` / `cipher` / `sni` 分，而 **`sni` 由请求方给** —— G118 刚刚为 `host` 挡过同一件事；「连接」那类要在 **L4 与 h1/h2/h3 三条入口各取一次数**，而**多处取数**正是 D18 / G66 要让它在结构上做不到的那个形状（批 M 为此把请求那两族死死钉在 `Record::finish` 一处）。⛔ 本条不预设答案。 | M3（对拍时会真的去读这些数）|
