...
| **M3** 对拍达标 | ⏳ **进行中** —— 第一刀（方法学）已落地：脚本、环境快照与判定口径在 [`bench/`](bench/README.md)，门禁里一格。⛔ **一个性能数字都还没有，而且是结构性地没有**（合格宿主不存在，G132/G139）|
| **M4** 发布 | ⏳ 未开工 |
...
⛔ ★ ★ ★ **本轮不产出任何性能数字，而且是结构性地不产出**：`bench/env-snapshot.sh`
先判宿主合格性，不合格时 `bench/verdict.sh` 不取最强者、不算门槛、不打 PASS / FAIL。
⇒ **`tests/bench/run.sh` 全绿 ≠ M3 达标**，它判的是「拿到合格宿主那天跑一遍就能出数」
这句承诺今天还成立。

⏳ **出数之前欠着两件**：合格的对拍宿主（**G132 记着开发机与生产机都不合格**）· 其余六类用例。
