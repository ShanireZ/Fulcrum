...
⏳ 其余五类用例（反代吞吐 2026-09-10 出了第一组读数；**高并发短连接** 2026-09-23 脚本与判据就绪，还没有读数）。⚠ G132 记的「开发机与生产机都
不合格」仍然成立 —— 那台合格宿主是第三台。
...
p99 = **9.22 ms**）。⏳ 阈值按 `G150` ② 等 owner 定，定下之前 ⛔ 不据它判任何事。
...
>
> ⚠ ⚠ **`G134`–`G150` 与 `D34` 的编号与措辞由 agent 拟**（`G133` 那条先例）。
> **每一条决策本身都是 owner 拍的。文字过没过目，⛔ 分成两段，别读成一句：**
>
