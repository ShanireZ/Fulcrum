...
| **D34** | ★★★ **判据 ④B（收敛）会被一个慢的被测掩盖。** 2026-09-06 第一趟真实对拍暴露：B 取的是**全场极差** `(最高−最低)/最高`，那一趟得 **0.8599**（远高于 `BENCH_MIN_SPREAD=0.05`）⇒ 判据一声没吭；⚠ ⚠ 而同一组数里**最快的两家 haproxy 与 nginx 只差 0.0071**，两套架构差异极大的实现落进 0.71% 以内 —— **那正是 B 声称要怀疑的形状**。⇒ 枢衡的慢把极差整个撑开，掩盖了快的那几家之间的收敛。★ 这条不是推理，是第一组真实数据给出的、可以推翻原设计的证据（`G142` 立 B 时就写明「0.05 是判断不是读数，第一次跑完之后要回头核」——回核回出来的就是这个）。**候选**：① 改判「最快的 k 家之间的收敛」（k 取几要定）；② 对**每一对**都判，任一对落在阈值内即作废（⚠ 最严，可能在四家里有两家天生接近时误作废）；③ 保持全场极差，另加一条独立的「最快两家收敛」判据。⚠ 与它成对的另一半：`BENCH_GEN_HEADROOM=0.90` **一次都没被执行过**（A 那一趟没打开）⇒ 那个数今天仍然验不了，而这一趟 haproxy/nginx 收敛在 ~54.5k **与「oha 在 2 个核上到顶」的形状一致** —— 本套东西证不了它们不是撞到了量具的天花板。★ 但那不动摇那一趟的 `FAIL`：若 54.5k 真是量具的顶，真实最强者只会更高 ⇒ 门槛只会更高。 | M3 出第二类读数之前 |
