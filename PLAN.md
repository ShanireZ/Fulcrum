...
| M | 观测 ②：**Prometheus 指标**。站点块里的终结指令 `metrics`（G116）· 文本格式自研零依赖（G117）· `no_site_match` 计数器结掉 D26（G118）· 站点标签取「实际匹配到的那条地址字面量」（G121）|
...
⏳ **观测那 2/3 已经排成批 M 与批 N**（方案见 [观测](docs/architecture/observability.md)，
决定见 §10 的 G116–G121）。⚠ **一格里装三件事，最容易被读成一件**；观测这一格今天是 **1/3，不是 ✅**。
...
| 「**两台**服务器全部由枢衡承载」 | ⏳ **这是运维动作，不是工程任务**。它不该挡住批 M / 批 N，但它是 M2 打勾的**唯一**硬条件 |
