...
> `G149` / `G150` 是 2026-09-07 才写的，**这四条都没有被它审过**，
> ⛔ 别把下面这三层读成覆盖了整个 `G134`–`G150`。
> ★ ★ `G149` 里还有一处**连数字都是 agent 拟的**（弃用过渡期具体几个 minor）——
> owner 拍的是「按面分档」那个形状，**那两个数字待过目**，就地标在该条里。
