...
⏳ **出数之前欠着三件**：合格的对拍宿主（**G132 记着开发机与生产机都不合格**）·
枢衡的线程数旋钮（**D33**）· 其余六类用例。
...
| **D33** | **枢衡的数据面没有线程数旋钮，⇒ 对拍今天只能把四家全钉成 1 个 worker。** 它走 pingora `ServerConf::default()`（`threads: 1`），而 DSL 与 CLI 里**一处都没有**（2026-09-05 实测，全仓 grep）。**G35** 当年拍的是「线程数按 service 角色定默认、**配置可覆盖**」—— 后半句至今没有落地面。⇒ 对拍要么四家全钉 1（今天唯一可对齐的值，`bench/case/static-throughput.sh` 的 `BENCH_WORKERS` 缺省就是它），要么先把旋钮做出来。⚠ 前者量出来的东西是**真的**，只是它回答的是「单 worker 下谁快」，而那不是任何人部署时的样子。⛔ 别读成「枢衡只能单线程」——读作「今天没有旋钮」。三条候选：① 全局指令（跟着 `grace_period` 那一族走）；② `serve` 的命令行选项；③ 按 service 角色算默认值再允许覆盖（G35 的原话）。 | M3 出数前 |
