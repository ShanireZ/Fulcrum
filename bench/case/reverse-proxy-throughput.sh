...
#   ⇒ 加第五个实现要改镜像，**撞 G36**（`G152` 把它的 M3 冻结扩到了对拍镜像）。
...
#
# ⚠ **枢衡 `file_server` 当时出局**的理由（2026-09-06 静态吞吐 7638.6 rps ⇒ 四家会一起被它压住
#   ⇒ 判据 ④B 收敛作废整类）按窗口二的数（19811.2，已高于这一类的最强者）**已不成立**；
#   选 `respond` 今天只靠上面那条「误差方向」站着 —— 见 bench/README.md「反代吞吐」一节。
set -euo pipefail
