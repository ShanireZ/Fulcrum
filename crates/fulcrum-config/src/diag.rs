...
impl DiagCode {
    ...
    /// ⇒ `id ""` 与「根本没写」是**同一个键**。
    /// ⚠ 最坏的现场恰恰是最像修好了的那一种：两条 `reverse_proxy` 的键相同，
    /// 有人给其中一条补了 `id ""` 想把它们分开 —— 而那一行什么都没改变。
    /// ⇒ ★ ★ **同键不是错误**（裁决 R6 ③ 第二轮 ⇒ G125），装载照样成功、
    /// 两条照样共享同一个覆盖格子 ⇒ **除了这一条诊断，没有任何东西会说话**。
    pub const EMPTY_PROXY_ID: DiagCode = DiagCode(42);
    /// `id` 不在取值域里：`[A-Za-z0-9_.-]`、长度 1–64（**M2 批 P**，裁决 R6 三之三）。
    ///
    /// # ★ ★ ★ 收紧的理由只有一条半，⛔ 别把它记成「基数风险」
    ///
    /// 1. **硬的那条 —— 与兜底记号撞车**：本仓用 `<other>` / `<none>` / `<unknown>` /
    ///    `<undeclared>` 表示「取不到 / 兜底」，靠的是**尖括号在真值里不可能出现**。
    ///    而在这一条之前，`id <none>` 是**合法**的。
    /// 2. **软的那半条 —— 可读性**：`id` 里的换行会让 `/stats` 与日志上那一行
    ///    在人眼里断成两行。
    ///
    /// ⛔ **不是**指标基数：G126 明写 `fulcrum_overrides_active` **无标签**，`id`
    /// 一个字都不进指标。★ 一个假理由会让下一个人理直气壮地把限制放宽回去。
    ///
    /// ⚠ **空串不走这一条**，它有 [`DiagCode::EMPTY_PROXY_ID`] 与自己那段解释
    /// （「`id ""` 与根本没写是同一个键」）—— 那句话是那条诊断的全部价值，
    /// ⛔ 不许被这条泛泛的「不合法」顶掉。
    ///
    /// ⚠ 这是一次**收紧**：在此之前能装上的某些配置以后装不上。M4 还没到
    /// （没有版本号、没有兼容性承诺，`PLAN.md` §7）⇒ 要收就现在收最便宜。
    ///
    /// ⚠ ⚠ **它只管 DSL 那条路。** 结构化配置（`POST /load`，G11 的公开入口）
