...
fn id_里有空格或尖括号或超长都装不上() {
    ...
    for (什么, 值) in [
        ...
        // ⚠ ⚠ **真的换行在 DSL 里表达不出来**，⛔ 别在这里问它：引号串不能跨行，
        //   词法层先回 `UNCLOSED_QUOTE`（`FUL-DSL-0003`）—— 那条诊断是对的，
        //   而它压根到不了 `proxy_id`。
        //   ⇒ 人想写换行时**实际写出来的**是 `\` + `n` 两个字符（词法层只认 `\"`
        //   与 `\\` 两种转义），而反斜杠不在取值域里 ⇒ 由这一条拦住。
        //   ★ 带**真**换行的那一格在 `fulcrum-runtime/tests/proxy_id.rs` 上：
        //   只有手搓的 JSON（`POST /load`）递得进来。
        ("想写换行、实际写出来的反斜杠", "\"a\\nb\"".to_string()),
        ("中文", "\"网页池\"".to_string()),
