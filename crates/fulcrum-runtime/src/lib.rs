...
/// 而不是 `a a b a`（**连发**）。
///
/// ⛔ ⛔ **不要相信「换成平滑加权只是换这一个函数体」** —— 这句话在这里写过一次，
/// **实测不成立**，它现在被换成了实情：
/// 平滑加权**本质有状态**（要每上游可变状态 + 临界区），而本函数今天是纯函数；
/// 且它的四个调用点里**有两个传进来的是哈希/随机值而不是序号**
/// （`ip_hash` 与 `random`），平滑加权对那两个**根本没有意义**。
/// ⇒ 真要换，第一件事是**决定它只接在 `round_robin` 上**，第二件事是拍板
/// 「请求热路径上能不能加一把锁」（今天 `ProxyTarget::cursor` 是无锁 `fetch_add`），
/// 而按 `PLAN.md` §8，那一批**要带着可复现的端到端测量开工**。
/// ★ ★ ★ 记住这条的形状：**一句写在注释里的代价估计没有任何门守着** ——
/// 它错了不会红，而下一个决定会建立在它上面。
///
...
pub struct SiteRt {
    ...
    /// 那是 **D26**（✅ 已由 **G118** 结案 —— 给 `no_site_match` 一个计数器），不是遗漏。
...
impl Runtime {
    ...
    /// 不会在登记处留下任何痕迹（G8）。
    ///
    /// ⚠ 建**成功**但调用方随后又不要它了时，传进来的那个 `layer` 会多出几格
    /// **空格子** —— 它们不计数、不进清单，并在下一次成功的 swap 时被清掉
    /// （见 [`OverrideLayer`]）。
    ///
    /// ⛔ **不要把上面那句读成「`/load` 被 409 拒掉时活的登记处会脏」** ——
    /// 这里写过那句话，**实测不成立**：`/load` 走的是 [`Runtime::build`]，而它
    /// 传的是一个**当场新建、随后丢弃**的 `OverrideLayer::new()`（见该函数），
    /// ⇒ 409 拒掉的时候，脏掉的是那个一次性的临时层，**活的登记处一个字节都没动**。
    /// ★ 事实上 `build_with_overrides` 配上**活的**登记层**没有任何生产调用方**
    /// （裁决 R8：它是 brief 要求的交付物，且判据 13 靠它成立）——
    /// 空格子那条路今天只有测试走得到。
    /// ★ ★ 记住这条的形状：**一句没有门守着的行为描述**，读起来完全成立，
    /// 而它描述的情形在真实调用路径上根本到不了。
