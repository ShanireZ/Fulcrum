...
// ── M2 批 N 任务 2.9：覆盖层的键与「同键共享一格」（裁决 R6 ③ 第二轮 ⇒ G125）──
//
// ⚠ ⚠ 这一组判据在任务 2.9 里**整体翻了向**，⛔ 一条都没有被删掉。
//   任务 2.8 照 R6 第一轮的口径断言「撞了 ⇒ 装不上」；owner 重拍之后
//   同一批事实要断言的是「撞了 ⇒ **两条拿到同一格**」。
//   ★ 翻向不是弱化：每一条仍然要能红，见下面第一条的说明。
//
// ★ ★ ★ 判定住在 **`fulcrum-runtime`**，因为键的第三格取的是
// `normalize_upstream` **之后**的串。⇒ 下面第一条（`myhost` vs `myhost:80`）
// 是这一组的核心：拿配置里的原文 token 建键的话，**它会静静地分成两格**，
// 而所有别的判据照样绿 —— 那就是一道假门。
...
/// DSL → 运行时图，**装不上就把每一条装载错误逐条打出来**。
///
/// ⚠ 任务 2.9 之后「同键」不再是装载错误 ⇒ 下面每一条判据的第一句都是「装得上」。
/// `rt()` 的 `expect` 只会说一句「运行时图应当能建起来」，不会说是谁挡的 ——
/// 而这一组一旦红，第一件要知道的事恰恰是**报了哪几条**。
fn 装得上(dsl: &str) -> Runtime {
    let o = compile_str("t.Fulcrumfile", dsl);
    assert!(!o.diagnostics.has_errors(), "{}", o.render_diagnostics());
    match Runtime::build(&o.config.unwrap()) {
        Ok(r) => r,
...
/// `proxy_key_fanout()` 的可读形态：`(站点名, id, 归一化后的上游地址, 管着几条)`。
fn 键们(r: &Runtime) -> Vec<(&str, &str, &str, usize)> {
    r.proxy_key_fanout()
        .into_iter()
        .map(|f| (f.site, f.id, f.addr, f.proxies))
        .collect()
}

#[test]
fn 核心判据_归一化之后两条拿到的键完全相同_哪怕两行原文长得不一样() {
    // ★ ★ ★ 本任务的核心判据，**从任务 2.8 原样翻向而来**（那时断言的是「装不上」）。
    //   它守的东西一个字都没变：**键的第三格取的是 `normalize_upstream` 之后的串，
    //   不是配置里那个原文 token**。`backend` 与 `backend:80` 在配置里是两个写法
    //   （`weight` 的地址比对就有意把它们当成两个），在运行时是**同一个上游**
    //   ⇒ 覆盖层里它们必须是**同一格**。
    // ⇒ 拿原文 token 建键的实现会把这一对算成两格：运维 `disable` 了一格，
    //   另一格上的同一台机器还在收流量，而没有任何东西会报错。
    // ★ 实测（任务 2.8 的 INJ-1 与本任务的 INJ-1）：把键换成配置原文时，
    //   这一组里**只有这一条**红，其余全绿。它不可替代。
    // ⚠ ⚠ ⚠ **上游名字有意不叫 `backend`**：本文件里别处的夹具用的就是 `backend`，
    //   而判据里的字面量与被测对象里的字面量重名时，那条判据可能是在验模板不是验数据
    //   （任务 2.8 的 INJ-13 实测栽过一次）。
    let r = 装得上(&两条("myhost", None, "myhost:80", None));
    // ① 先钉住「两条 `reverse_proxy` 都还在」—— 否则「只有一格」可以靠**少一条**满足。
    assert_eq!(r.keyed_proxies().len(), 2, "两条 reverse_proxy 都该在");
    // ② ★ 两条**拿到的键完全相同** ⇒ 覆盖层里只有一格，而那一格管着两条。
    assert_eq!(
        键们(&r),
        vec![("a.com", "", "myhost:80", 2)],
        "`myhost` 与 `myhost:80` 归一化后是同一个上游 ⇒ 必须是同一格，且管着两条"
    );
    // ③ 键的第三格逐字是**归一化之后**那个串（不是 `myhost`，也不是别的什么）。
    for p in r.keyed_proxies() {
        assert_eq!(
            p.target.upstreams[0].addr, "myhost:80",
            "键的第三格必须是归一化之后的串"
        );
    }
}
...
#[test]
fn 同一站点两条指着同一个上游而都没写_id_共享同一格() {
    // 主场景（任务 2.8 里它断言「装不上」）：两条原文就一模一样。
    // ⇒ 现在断言的是**装得上**，且两条落在同一格里。
    let r = 装得上(&两条("10.0.0.1:8080", None, "10.0.0.1:8080", None));
    assert_eq!(
        键们(&r),
        vec![("a.com", "", "10.0.0.1:8080", 2)],
        "两条都没写 id ⇒ 键相同 ⇒ 同一格，一次 disable 一起生效"
    );
}

#[test]
fn 一个后端挂在四组路由后面_装得上而且共享同一格() {
    // ★ ★ 这是 `tests/serve/run.sh` 那份夹具的形状（`/api` `/rw` `/cached` `/hc`
    //   四条 `reverse_proxy` 指着同一台机器）—— 反代最常见的写法。
    //   任务 2.8 照第一轮口径把它拒了，serve / h3 / log / encode **四个端到端场景
    //   当场全部装不上**，那正是 owner 重拍的原因。
    // ⇒ 这一条把那个形状钉在单测里：端到端场景红的时候，先看它红没红。
    let mut dsl = String::from("a.com {\n");
    for p in ["api", "rw", "cached", "hc"] {
        dsl.push_str(&format!(
            "  handle /{p}/* {{\n    reverse_proxy 10.0.0.1:8080\n  }}\n"
        ));
    }
...
#[test]
fn 给其中一条写了_id_就分成两格() {
    // ★ ★ **反向判据**：少了它，一个「所有 `reverse_proxy` 都塞进同一格」的实现
    //   照样能让上面每一条绿 —— 而那样 R6 的全部价值（写 id 就能细分）就没了。
    let r = 装得上(&两条(
        "10.0.0.1:8080",
        None,
        "10.0.0.1:8080",
    ...
    assert_eq!(
        键们(&r),
        vec![
            ("a.com", "", "10.0.0.1:8080", 1),
            ("a.com", "pool_web", "10.0.0.1:8080", 1),
        ],
        "写了 id 就分得开：两格，各管一条"
    );
    // 两条都写、且写的不一样，也分得开。
    ...
    assert_eq!(
        键们(&r),
        vec![
            ("a.com", "a", "10.0.0.1:8080", 1),
            ("a.com", "b", "10.0.0.1:8080", 1),
        ]
    );
}

#[test]
fn 两条写了同一个_id_照样共享同一格() {
    // ★ 反向的反向：`id` 不是「写了就分开」的标记，它是键里的一格。
    //   ⚠ 少了这一条，一个「`id` 非空就各占一格」的实现能让上面每一条都绿。
    let r = 装得上(&两条(
        "10.0.0.1:8080",
        Some("same"),
        "10.0.0.1:8080",
    ...
    assert_eq!(
        键们(&r),
        vec![("a.com", "same", "10.0.0.1:8080", 2)],
        "同一个 id 指着同一台机器 ⇒ 仍然是同一格"
    );
}

#[test]
fn 两条指着不同的上游而都没写_id_是两格() {
    // ★ ★ **反向判据**：少了它，一条「同一个站点里的上游全归一格」的实现照样能让
    //   主场景绿 —— 而那会让 `disable` 一台机器把整个站点摘掉。
    let r = 装得上(&两条("10.0.0.1:8080", None, "10.0.0.2:8080", None));
    assert_eq!(
        键们(&r),
        vec![
            ("a.com", "", "10.0.0.1:8080", 1),
            ("a.com", "", "10.0.0.2:8080", 1),
        ],
        "上游不同就是两格"
    );
}
...
#[test]
fn 不同站点里的同一台机器是两格() {
    // 键的第一格是站点名（`SiteRt::name`，裁决 R6 ④）⇒ 跨站点不并。
    let r = 装得上(
        "a.com {\n  reverse_proxy 10.0.0.1:8080\n}\nb.com {\n  reverse_proxy 10.0.0.1:8080\n}\n",
    );
    assert_eq!(
        ...
        vec![
            ("a.com", "", "10.0.0.1:8080", 1),
            ("b.com", "", "10.0.0.1:8080", 1),
        ],
        "两个站点各自指着同一台机器 ⇒ 两格，运维摘的是「这个站点块里的那台」"
    );
}

#[test]
fn 同一条_reverse_proxy_里把同一个地址写两遍只算一条() {
    // ★ 边界（`ProxyKeyFanout::proxies` 的文档写着）：那两个 `Upstream` 是**同一条**
    //   `reverse_proxy` 上的同一台机器，一次 `disable` 把它们一起摘掉本来就是要的语义
    //   ⇒ 计数里**不该**因此显示「这一格管着两条 reverse_proxy」——
    //   那会让 `/stats` 对一个完全正常的写法喊狼来了。
    let r = 装得上("a.com {\n  reverse_proxy 10.0.0.1:8080 10.0.0.1:8080\n}\n");
    assert_eq!(
        键们(&r),
        vec![("a.com", "", "10.0.0.1:8080", 1)],
        "同一条里写两遍只算一条 reverse_proxy"
    );
}

#[test]
fn 藏在容器里的_reverse_proxy_也有键也会被数进同一格() {
    // ⚠ ⚠ 走法漏掉容器的表现是**两件事一起静默**：那条 `reverse_proxy` 没有键
    //   （覆盖指不到它），而 `/stats` 里那一格的计数也少一条。
...
fn 键的三格都在_keyed_proxies_上取得到() {
    ...
    assert_eq!(ps[1].id, "");
    // 第三格：**归一化之后**的上游地址。
    // ⚠ 配置那一行上的原文 token（`backend`）在运行时里**没有留档** ——
    //   任务 2.8 存过一格 `raw_addr`，唯一的消费者是撞键那条拒绝报文，
    //   任务 2.9 拆掉拒绝之后它就没有消费者了，⇒ 删。
    ...
    // ★ `proxy_key_fanout` 走的也是这一份 —— 三格逐字对得上。
    assert_eq!(
        键们(&r),
        vec![
            ("a.com", "", "10.0.0.2:8080", 1),
            ("a.com", "pool_web", "backend:80", 1),
        ]
    );
}
...
fn 没写_id_的那条在运行时是空串而不是别的什么() {
    // ⚠ 空串是键里正正经经的一格。改成「`None` 就不查」会让主场景静静漏到别的分支上。
    let r = rt("a.com {\n  reverse_proxy 10.0.0.1:8080\n}\n");
