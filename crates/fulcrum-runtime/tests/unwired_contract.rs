...
fn unwired_for(dsl: &str) -> Vec<&'static str> {
    ...
    rt.unwired_in_use(&cfg).iter().map(|(k, _)| *k).collect()
}
...
",
    ...
    //   所以**一条都不该报**。
    //   ⚠ 它在这张表上只待了几个小时（同日被抓出、同日接上），但那一趟暴露了
    //   这张表自己的盲区：`unwired_in_use` 此前**只扫站点、不扫全局选项** ——
    //   一张只扫一半的表，在它没扫的那一半上与不存在没有区别。
    //   ★ 这一行现在守的是那个盲区还通着：全局选项那一半**真的被扫到了**。
    assert_eq!(
