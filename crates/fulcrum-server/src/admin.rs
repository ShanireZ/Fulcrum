...
struct StatsResponse {
    ...
    /// 缓存后端的读数。`Some` 时哪怕 `bytes`/`entries` 都是 0，也是一次真读数，
    /// 不是缺省值冒充。
    ///
    /// ⚠ ⚠ **当前构型下它恒为非 `null`**（owner 2026-09-03 拍板：`Option` 保留，
    /// 文档改成实话）。理由是**结构性的**：唯一的生产装配点无条件构造缓存句柄，
    /// 再无条件包成 `Some` ⇒ **真二进制吐不出 `"cache": null`**。
    /// ⇒ ⛔ **运维不要为 `null` 写分支** —— 那会是一段永远不执行的代码。
    /// ★ `Option` 留着是给「将来可能不带该后端的构型」占位的，**不是**今天的状态描述；
    /// 而 `None` 今天只出现在单测的装配里。判据在 `tests/stats/run.sh`
    /// （⚠ 它只证得了**当前构型**，那也正是这条契约今天的全部内容）。
    cache: Option<CacheStat>,
    /// 已装载证书的清单。`Some(vec![])` = 有解析器但零证书（比如整份配置没有一个
    /// TLS 站点）—— ★ 那与「没有解析器」是不同的状态。
    ///
    /// ⚠ ⚠ **当前构型下它同样恒为非 `null`**，理由与 `cache` 逐字相同（同一处装配、
    /// 同样无条件包 `Some`）。⇒ 今天分得开的只有 `[]` 与非空，⛔ 不是 `null` 与非 `null`。
