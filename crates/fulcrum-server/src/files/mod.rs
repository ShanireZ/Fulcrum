...
//!
//! ## ⚠ 阻塞 IO 走 `spawn_blocking`，但**只在真的要等 I/O 时**
//!
//! 直接在 async 里做文件 IO 会**占住一个工作线程** —— 页缓存命中时只是几微秒，
//! 而一次冷读（或一块坏盘）能把它占到毫秒级，那时整个 worker 上的其它连接一起卡住。
//! ⇒ 会等 I/O 的活必须进 [`tokio::task::spawn_blocking`]。
//! ⚠ ⚠ ★ **但反过来不成立**：把**不会等**的活也塞进去是纯亏损，而这正是
//! 2026-09-06 之前的写法。
//!
//! ★ ★ ★ **批 F 当初写的是「不为吞吐做优化，性能是 M3 的事」——
//! M3 发生了，而它给出的数字推翻了那时的实现。** 第一趟真实对拍：
//! 枢衡静态吞吐 **7638 rps**，nginx / HAProxy **54000+**，⇒ `FAIL`。
//!
//! 根因量了两轮，⛔ 一次都不是猜的（同进程、同负载、同二进制，只换终结指令）：
//!
//! **第一轮**只数了上下文切换次数，据此把三次 `spawn_blocking`
//! （`stat` / `open` / `read`）合成一次 —— 切换/请求 5.87 → 1.71，**预测中了**，
//! ⚠ ⚠ **而 rps 只从 20623 到 24432（+18%）**。
//!
//! **第二轮**（`perf`，并发 200，2 个核）问的是「那些 cycle 到底花在哪」：
//!
//! | 每请求 | `file_server` | `respond`（纯内存） |
//! |---|---:|---:|
//! | cycles | 250,041 | 96,098 |
//! | instructions | 91,107 | 69,178 |
//! | **IPC** | **0.36** | **0.72** |
//! | **cache-miss** | **7,385** | **1,578** |
//! | 上下文切换 | 1.46 | 0.007 |
//! | CPU 利用 | 1.69 核 | 1.99 核 |
//!
//! 按族合计：调度 / 唤醒 / 系统调用入口 **20.94%**（`respond` 9.83%）、
//! **文件系统本身只有 3.16%**、**本模块自己的业务逻辑只有 2.14%**。
//!
//! ⇒ ★ ★ ★ **成本不在「做文件那件事」，在「把每个请求在工作线程与阻塞线程
//! 之间弹一次」。** 而它**不表现为切换次数** —— 表现为 cache-miss 4.68 倍、
//! IPC 掉一半、两个核还空着 15%。**次数砍掉三分之二，每一次的破坏力没变**，
//! 所以只换来 +18%。⛔ 这是本模块最容易重犯的一次误判：量纲选错了，
//! 而选错的那个量纲**看上去完全像个正经判据**。
//!
//! ⇒ 现在 [`probe_file`] **先在本线程上试一次**
//! （`open` + `fstat` + `preadv2(RWF_NOWAIT)`），页缓存命中就整趟零跨线程；
//! 只有内核回「这一次要等」时才付那一次 `spawn_blocking`。
//! 而 [`send_bytes`] 从内存发。
//!
//! ⚠ ⚠ ★ **快路径认不认由文件系统说了算**：实测 ext4 认，
//! **overlayfs 与 tmpfs 回 `EOPNOTSUPP`** ⇒ 站点根落在镜像层或 tmpfs 上时
//! 本条**恒不生效**，行为与改动前逐字相同。⛔ 引用本节的收益时不许丢掉这一句。
//!
//! ### 改完之后量到了什么（诊断台，⛔ 不是性能读数）
//!
//! | 并发 200 | 改前 | 改后（ext4）| 改后（overlayfs）|
//! |---|---:|---:|---:|
//! | rps | 28452–29591 | **41023–44361** | 28246–29034 |
//! | 自愿切换/请求 | 1.70 | **0.006** | 1.66–1.73 |
//! | 阻塞线程 | 74–83 | **6** | 56–63 |
//! | 调度族占比 | 20.94% | **9.69%** | — |
//! | CPU 利用 | 1.69 核 | **1.99 核** | — |
//!
//! ⇒ **那次 handoff 的代价整个消失了**：切换数、线程数、调度占比与 CPU 利用
//! 四项全部与 `respond` 持平。并发 1 那一格 5492 → 10065（+83%），
//! p50 由 0.173 ms 到 0.095 ms。
//!
//! ⚠ ⚠ ★ ★ ★ **而它仍然只有 `respond` 的一半**（44k 对 91k），⛔ 别读成
//! 「那个 `FAIL` 修好了」—— 合格宿主上的门槛是 **49078**。
//! 写这段时立过一条可证伪预测「改完落在 70–90k」，**实测 41–44k，预测错了**。
//!
//! 剩下那 2.1 倍**不再是调度，是文件那几个系统调用本身**（每请求实测）：
//!
//! | | `file_server` | `respond` |
//! |---|---:|---:|
//! | 系统调用总数 | **6.81** | **3.08** |
//! | 其中 `openat` | 0.943 | 0 |
//! | 其中 `recvfrom` | 0.943 | 0.965 |
//! | 指令数 | 103,148 | 67,678 |
//!
//! ⇒ 网络那一侧**每请求逐条相同**，多出来的 **3.73 次**全是
//! `open` / `fstat` / `preadv2` / `close`；而指令数正好多 **35,470**
//! ⇒ 按每次系统调用 ~9k 指令（seccomp + spectre 缓解 + `openat` 的整条路径解析）
//! **它们把差额解释满了**。★ 最贵的是 `openat`：每请求重走一遍路径解析与 dentry 查找。
//!
//! ⏳ ⇒ **再往下只有一条路：别再做这些系统调用**（把打开的文件或它的字节缓存起来，
//! nginx `open_file_cache` 那一路）。⛔ 那是一次独立的、要 owner 拍板的设计
//! （失效语义、内存上限、一整类「发陈旧内容」的缺陷），**本轮有意不做**。
//!
//! ⚠ ⛔ **大文件、预压缩旁文件、目录索引仍然走原来的分块路径** ——
//! `spawn_blocking` 存在的理由在那些场景仍然成立。
//! ⚠ 另有一处**没有**被本次改动碰到的每请求 `spawn_blocking`：
//! [`within_root`]（只在站点写了 `follow_symlinks false` 时走）。已登记，⛔ 本次不动。
...
pub(crate) async fn serve(
    ...
    // ── 6. metadata（+ 小文件顺手整读；页缓存命中时零跨线程）──────────────
    //
    // ★ ★ 为什么是「能不等就别跨线程」：见本模块头部那张 `perf` 表。
...
/// 探一次文件：**能不等就在本线程做完，要等才跨线程**。
///
/// ★ ★ ★ 它取代的是**每请求一次无条件的跨线程 handoff**（更早还是三次）。
/// 那次 handoff 的真实代价见本模块头部那张 `perf` 表 —— ⛔ 别再用
/// 「上下文切换次数」当量纲，2026-09-06 已经用那个量纲误判过一次。
///
/// 判据由内核给：[`Probe::open_nowait`] 走 `preadv2(RWF_NOWAIT)`，
/// 「小」的定义因此是**「内核说这一次不用等」**，⛔ 不是一个写不出反证的字节阈值。
///
/// ⚠ 需要读多少的界取 [`CHUNK`]，⛔ **不引入新常量**：那是本模块已有的分块粒度，
...
async fn probe_file(p: &Path) -> Option<Probe> {
    // ── ① 先在**本线程**上试一次，一次跨线程都不做 ─────────────────────────
    //
    // ★ ★ ★ 2026-09-06 剖面量出来的：做文件那件事只值 **3.16%** 的 CPU，
    //   本模块自己的业务逻辑只值 **2.14%** —— 而把每个请求在工作线程与阻塞线程
    //   之间弹一次的代价占了余下几乎全部。⚠ 它**不表现为切换次数**
    //   （每请求只剩 1.46 次），表现为每请求 cache-miss **4.68 倍**、
    //   IPC 由 0.72 掉到 0.36、两个核还空着 15%。
    //   ⇒ 上一轮把切换次数砍掉三分之二只换来 +18%：次数减少了，
    //   **每一次的破坏力没变**。
    if let Some(probe) = Probe::open_nowait(p.to_path_buf(), CHUNK as u64) {
        return Some(probe);
    }

    // ── ② 到这里才是真的要等 I/O ──────────────────────────────────────────
    //
    // ★ 这正是 `spawn_blocking` 该出场的场景（别让一次冷读占住工作线程），
    //   ⇒ 它没有被删掉，只是**不再为每一个页缓存命中付一次**。
    // ⚠ ⚠ 这条路也覆盖「文件系统不认 `RWF_NOWAIT`」（overlayfs / tmpfs 实测如此）
    //   ⇒ 那种部署上的行为与本次改动前**逐字相同**，代价是每请求多一次
    //   失败的 `preadv2`（半微秒量级）。⛔ 不缓存这个判断：一个进程可以有
    //   多个站点根落在不同的文件系统上，而一份记错了的全局状态没有任何东西会说。
    // ⚠ `open` 失败也走这里 —— 多一次注定失败的 `open`，只发生在 404 那条路上。
    //
    // ⚠ ⚠ ★ ★ **它在 [`serve`] 那一层是「性能」承重，在本函数这一层是「正确性」
    //   承重 —— 而这两件事 2026-09-06 被混成一句话，代价是它整整一天没有门守着。**
    //
    //   · `serve` 那一层：删掉它照样发得出文件（本函数回 `None` ⇒ `stat()` ⇒
    //     `Probe::meta_only` ⇒ [`stream_body`] 分块路径）⇒ 少了它是
    //     **1 次跨线程变 3 次**，而不是发不出文件。**实测**（本机 ShanireHomePC）：
    //     把它整个删掉，`FILES_ONLY` **RC=0 全绿**，
    //     且 365 条单测里**只有下面那一条**红。
    //   · 本函数这一层：契约是「小文件带回全部字节」，而在一个**不认 `RWF_NOWAIT`**
    //     的文件系统上（overlayfs / tmpfs 实测如此），唯一做得到的就是这条回落。
    //     ⇒ 由 `probe.rs` 的
    //     `文件系统不认_nowait_时_probe_file_仍然必须带回全部字节` 守着，**删掉即红**。
    //
    // ⚠ ⛔ **仍然没有门的那一半**：「它只花 1 次跨线程」。把它重写成正确、
    //   却要三次跨线程的实现，上面那条测试照常绿 —— 那一半今天只由诊断台的读数守着。
    let p = p.to_path_buf();
