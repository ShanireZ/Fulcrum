...
async fn probe_file(p: &Path) -> Option<Probe> {
    ...
    //
    // ⚠ ⚠ ★ **这条回落是「性能」承重，⛔ 不是「正确性」承重** —— 2026-09-06 实测：
    //   把它整个删掉，`tests/files` **一格都不红**。因为 [`serve`] 里还有第二条回落
    //   （本函数回 `None` ⇒ `stat()` ⇒ `Probe::meta_only` ⇒ [`stream_body`] 分块路径），
    //   少了它是「1 次 handoff 变 3 次」而不是「发不出文件」。
    //   ⇒ ⛔ **别因为「删了也全绿」就删掉它**，那正是它今天没有门守着的地方。
    let p = p.to_path_buf();
