...
mod tests {
    ...
    /// ★ ★ ★ **`probe_file` 那条回落，在这一层是「正确性」承重。**
    ///
    /// ⚠ ⚠ 2026-09-06 它被整个记成「**性能**承重」，依据是一次注入反证：
    /// 把 `probe_file` 里那次 `spawn_blocking(open_blocking)` 删掉，**整趟门禁
    /// RC=0、一格都没红**。⇒ 于是它当天没有任何门守着。
    ///
    /// ★ 那个判断在 [`super::super::serve`] 那一层是对的：`serve` 还有第二条回落
    /// （`probe_file` 回 `None` ⇒ `stat()` ⇒ [`Probe::meta_only`] ⇒ 分块路径）
    /// ⇒ 少了它是**1 次跨线程变 3 次**，而不是发不出文件。
    ///
    /// ⛔ **但 `probe_file` 自己的契约不是那个。** 它答的是「这个小文件的字节
    /// 拿到了没有」，而在一个**不认 `RWF_NOWAIT`** 的文件系统上（overlayfs /
    /// tmpfs 实测如此），唯一做得到这件事的就是那条回落。
    /// ⇒ **在这一层它删掉即红**，⛔ 不需要任何性能量纲。
    ///
    /// ⚠ ⚠ ★ **诚实的射程**：本条只钉住「回落在、且把事情做对了」，
    /// ⛔ **钉不住「它只花 1 次跨线程」** —— 有人把它重写成正确但要三次跨线程的
    /// 实现，本条照常绿。那一半今天仍然**只由诊断台的读数守着**，
    /// ⛔ 别把本条读成「那条回落全都有门守着了」。
    ///
    /// ★ 它不会退化成恒真：「不认」那个根确实在场，由
    /// [`每趟必须两个方向都真的走到`] 保证 —— 少了那一条，本条会在一台所有根都
    /// 认 `RWF_NOWAIT` 的宿主上**静默地什么都不测**，所以下面那句 `assert!` 也留着。
    #[tokio::test]
    async fn 文件系统不认_nowait_时_probe_file_仍然必须带回全部字节() {
        let 不认的: Vec<PathBuf> = 本趟要跑的根()
            .into_iter()
            .filter(|r| !文件系统认不认_nowait(&Tmp::under(r, "回落-探能力").0))
            .collect();
        assert!(
            !不认的.is_empty(),
            "本趟一个「不认 RWF_NOWAIT」的根都没有 ⇒ 快路径处处命中，\
             那条回落一次都走不到，本判据什么都没测到（⛔ 不是「通过了」）"
        );

        for root in 不认的 {
            let t = Tmp::under(&root, "回落");
            // ⚠ 非空且远小于 `CHUNK`：它必然走「该整读」那一支，
            //   ⇒ 快路径回 `None` 之后，字节只能由回落带回来。
            let (p, body) = t.file("f.bin", 4096);
            let probe = super::super::probe_file(&p).await.expect(
                "`probe_file` 在不认 RWF_NOWAIT 的文件系统上回了 None —— \
                 那条 spawn_blocking(open_blocking) 回落多半被删了。\
                 ⚠ 它在 serve 那一层只是性能（还有第二条回落兜着），\
