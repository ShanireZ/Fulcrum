...
- **临时覆盖层不持久化。** 进程重启之后它被清空；全量 load 之后清不清空由
  ...
  地消失。
- ★ **但它永远可见**，分两个出口，**形式不同、内容互补**：
  · `POST /load` 与 `POST /runtime` 的**每一次响应**都在正文末尾带一行
    「当前有 N 项临时覆盖生效中（其中 M 项悬空）」（裁决 R11）；
  · `GET /stats` 的 `overrides` 一节把每一项**逐条列出**（三格键原样、`disabled`、`weight`、`dangling`）。
    ⚠ `/stats` 的正文是 JSON，**有意不追加**那一行纯文本计数 —— 追加会把它撕成不合法的 JSON。
  · 另有无标签 gauge `fulcrum_overrides_active`（G126）给抓取端，取数与 `/stats` 同源。
- ⛔ **逐项里没有「谁」，也还没有「什么时候」。**「谁」在 G14 之下**结构上不存在**
  （管理面走 Unix socket + `0600`，访问控制交给文件系统 ACL，请求不携带任何身份）
  ⇒ 这一格永远不会有。「什么时候」是**尚未做**、已登记的候选（裁决 R6），
  ⚠ 越晚做越贵：加字段要同时改 `/stats` 的 JSON 契约与挂在它上面的场景判据。
