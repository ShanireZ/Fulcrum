...
- ★ **但它永远可见**，分两个出口，**形式不同、内容互补**：
  ...
  · `GET /stats` 的 `overrides` 一节把每一项**逐条列出**（三格键原样、`disabled`、
    `weight`、`dangling`、`set_at_unix`）。
    ⚠ `/stats` 的正文是 JSON，**有意不追加**那一行纯文本计数 —— 追加会把它撕成不合法的 JSON。
  · 另有无标签 gauge `fulcrum_overrides_active`（G126）给抓取端，取数与 `/stats` 同源。
- ⛔ **逐项里没有「谁」。** 在 G14 之下它**结构上不存在**（管理面走 Unix socket + `0600`，
  访问控制交给文件系统 ACL，请求不携带任何身份）⇒ 这一格永远不会有。
- ✅ **「什么时候」已经做了**（裁决 R6 留下的那条，owner 拍板）：`set_at_unix`，**绝对 Unix 秒**。
  ★ ★ 语义是「**最后一次改动**」而不是「首次被覆盖」—— 运维问的那句话是
  「这台机器摘掉多久了」，问的是**当前形态**的年龄；先改 `weight` 再 `disable` 时，
  取「首次」的读数会让人以为它两天前就摘了，而其实两分钟前才摘。
  ⚠ 时钟取**墙钟**，代价写在明处：墙钟会跳（NTP 校正）⇒ 极端情况下两项覆盖的先后
  与时间戳顺序可以不一致。⛔ 不为此引入第二个单调时钟 —— 那会让 `/stats` 出现两种时间。
  ⛔ **不是「已经过了多少秒」**：相对秒读起来完全正常，而它在两次抓取之间含义会变（R5）。
  ⚠ 它**不进指标**（G126 已经拍过：「是哪几项」本来就该来 `/stats` 看）。
