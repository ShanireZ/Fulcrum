...
> ★ **DNS 定期重解析针对的是 nginx 的一个经典事故源**：上游域名**默认**只在启动时解析一次，后端 IP 变了必须 reload。
> ⚠ 2026-09-24 更正：1.27.3 起 nginx 开源版可以在 `upstream` 的 `server` 上显式写 `resolve`（要另配 `resolver` 与共享内存 `zone`）做动态重解析 ⇒ 差别在**默认行为与配置门槛**，⛔ 不是「nginx OSS 做不到」。
