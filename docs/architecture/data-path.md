...
| **HTTP/3（UDP + QUIC）** | **自建 `Service`**，⚠ ~~`quinn` + `h3`~~ → **`quiche`**（**G103**）| ✅ **（批 J）已落地并接线**：有 `tls` 的端口自动在**同端口**听 UDP（G110），h1/h2 响应带 `Alt-Svc`。★ Pingora（0.8 与 2026-09-24 rebase 上去的 0.9 都一样）**不带 HTTP/3**；★ 自建的是入口/连接归属/路由汇合，**协议栈本体用 `quiche::h3`**（G105）|
...
## ⚠ pingora 0.9.0 起，h1 / h2 入口会在路由之前拒掉几类请求（2026-09-24 rebase 带进来，接受、不改）

上游 0.9.0 收紧了 HTTP 解析（request-target 分类统一、歧义 authority 一律拒）。这几类请求**到不了枢衡的路由层**：

| 入口 | 被拒的形状 | 客户端看到什么 | 枢衡这边留下什么 |
|---|---|---|---|
| h1 | 重复的 `Host`；`Host` 或 URI 带 userinfo；absolute-form 的目标与 `Host` 不完全相同（大小写、端口写法不同都算）；有歧义的 absolute-form（`http:host`、authority 里有反斜杠）| ⚠ **连接直接断开，⛔ 不回 400** | ⚠ **什么都没有**：`read_request()` 出错即返回，访问日志与指标里都没有这条 |
| h2 | 重复的 `Host`、userinfo、`Host` 与 `:authority` 不一致、两者都缺、`:path` 是 absolute-form 或有歧义、`:path` 里有 CR / LF | pingora 回 400 或重置这条流；同一连接累计 32 条后整条断开 | 同上，收不到 |
| h3 | **不走这套校验** | —— | ⚠ 于是 h1/h2 与 h3 在「重复 `Host` / authority 不一致」上处理不同（已知差异，登记未改）|

另外三条会被读错的变化：

- absolute-form 请求的 `uri.path()` 从「整段 URL」变成「只有路径」，`#` 片段一律去掉 ⇒ 路由按路径匹配（更正确），访问日志里的 `uri` 变短；转发给上游的 `raw_path()` 不变。
- 反方向：值**相同**的重复 `Content-Length`（含 `5, 5` 这种逗号写法）从「拒绝」改成「接受」，上游响应里也一样。
- 上游方向：请求目标里有 NUL / LF / CR / 空格时，写上游请求头失败 ⇒ 枢衡回 `all_upstreams_down`；上游响应头里的折行（obs-fold）被规范成单个空格。
  另外，h1 上游的响应**没读完**的连接不再放回池里复用（`1ed40d4`）⇒ 重测连接池时复用率若比 0.8.1 低，先查这一条。

★ HTTP/1 下游 pipelining 在 0.9.0 里是**可选开启、缺省关**，缺省行为与 0.8.1 相同（有多读字节时不复用连接）。

h2 入口还有两条（设计里没列、rebase 当天从上游日志里补查到的）：

- ⚠ **h2 默认上限收紧了**（`90890bb`）：`HttpServerApp::h2_options()` 返回 `None` 时，pingora 改用
  「每连接最多 **100** 条并发流 + 解码后的头部表最多 **64 KiB**」。枢衡没覆写 `h2_options()` ⇒ 直接吃到这两个数
  （0.8.1 时是 h2 crate 0.4 的默认：**不通告**并发流上限 = 不限，头部表 16 MiB）。
  ⇒ 一条 h2 连接上要同时开超过 100 条流的客户端（典型是 gRPC）：守规矩的排队，不守的那几条流被 h2 当场重置。
  ★ owner 2026-09-24 拍：**接受 0.9.0 的默认**，不覆写 `h2_options()`。
- ★ 停机时 h2 连接里**已经收到的流会被处理完**再断（`aece993`，RFC 9113 §6.8）；0.8.1 收到停机信号就直接退出接收循环。

## ★ h1 / h2 监听的写缓冲是 16 KiB，不是 pingora 缺省的 1460 字节（G155）

pingora 给每条接入连接套一层带缓冲的流：读缺省 64 KiB，写缺省只有 **1460 字节**。
h1 写响应是先把头 `write_all` 进缓冲、再写正文 ⇒ 「缓冲里的 + 这次的」一超过容量，
头就先被冲出去、正文再写穿 ⇒ 头 + 约 4 KiB 正文的响应是**两次 send、两个 TCP 段**
（nginx / haproxy 是一次）。⇒ 枢衡的 L7 数据面监听（明文与 TLS）把写缓冲设成 **16 KiB**
（上游的公开接口 `ListenerConfig::l4_buffer`，⛔ 没动 fork；落点是 `crates/fulcrum-server/src/lib.rs` 的 `L7_L4_BUFFER`）。

| | |
|---|---|
| 覆盖到的 | 头 + 正文不超过约 16 KiB 的响应一次发出（TLS 时是头与正文两个记录一起发）|
| 覆盖不到的 | 更大的响应仍然先冲头、再写穿。要连它一起治，得在 fork 里把头与第一块正文合成一次发送（方案 B）—— **只登记、不排期**（`G155`）|
| 代价 ① 内存 | 写缓冲在建连时整块分配 ⇒ 每条连接多约 15 KiB（读缓冲本来就是 64 KiB）|
| 代价 ② 时延 | pingora 只对带 `Content-Length` 的正文缓冲，chunked 与读到关闭的正文每写必 flush ⇒ SSE 一类不受影响。分几段写的 CL 正文（比如反代一个慢上游）最多在缓冲里多攒 16 KiB，攒满或写完最后一段才发 |
| 没改的 | 管理面的 Unix socket 仍是 pingora 缺省；L4 透传与 HTTP/3 本来就不走这层缓冲 |
| 判据 | [`tests/serve/run.sh`](../../tests/serve/run.sh) 第 10 节：客户端读自己 socket 的 `TCP_INFO`，4 KiB 的响应明文与 TLS 都只许 1 个带数据的段；32 KiB 那条是内置反证（仍须 ≥ 2）。⚠ 只量了 h1；h2 走同一层缓冲，没单独量 |
