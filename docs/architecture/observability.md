...
sources:
  ...
  - id: plan-11
    resource: /references/plan.md
    title: PLAN.md §10 G149（版本与兼容性策略，原 D9）、G123（缓存事件的折叠口径与边界，D31 结案）、G124（status_class 的第 6 个值，D30 结案）、G122（连接类与 TLS 类指标，D32 结案）、G127（标签值取不到时记 <unknown>）、G128（h3 的 TLS 三格走 fork 改动 14，D27 + D28 结案）
---
...
## `fulcrum_build_info{version}` 报的是**构建身份**，⛔ 不是版本承诺

值由 [`crates/fulcrum-server/build.rs`](../../crates/fulcrum-server/build.rs) 在编译期算出（`G141`），
取值链是 `FULCRUM_BUILD_ID` → 读 `.git` 推出提交号 → 显式 `unknown`。

⛔ ★ **取不到时是 `<包版本>+unknown`，不是裸的 `0.0.0`** ——
「读不到」与「读到了 0.0.0」必须区分得开。看到 `+unknown` 就说明这次构建**没有**身份可报
（多半是 `.git` 不在上下文里，例如 `Dockerfile.musl-product` 那一趟），
⇒ 那种构建要靠 `FULCRUM_BUILD_ID` 喂真值。

★ 用 semver 的**构建元数据**语法（`+` 之后那一段不参与版本先后比较）
⇒ 语法本身就在声明「这是身份，不是承诺」。**承诺**是另一件事，见
[兼容性分类法](/governance/compatibility.md)：分类法今天生效，承诺从 M4 起。

⚠ **它不判工作树脏不脏** —— 同一个提交上带着不同未提交改动的两次构建，报出来的身份相同。
