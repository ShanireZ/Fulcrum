...
| 面 | 形态 |
|---|---|
| **指标** | Prometheus 端点。✅ **已落地（批 M，G116–G118/G121；批 N 又加了覆盖层一族）** —— 覆盖请求、上游、缓存、证书、ACME 签发与临时覆盖层，见下面「指标」一节。⚠ **连接**与 **TLS** 两类一个族都没有，已登记为 [`PLAN.md`](../../PLAN.md) §11 的 **D32** —— ★ 那不是漏做：G116 拍板的清单里本来就没有这两类 |
...
| | 怎么取 | 谁 |
|---|---|---|
| **事件点记账** | 发生一次就往进程级注册表里加一笔，渲染只是把数抄出来 | 上表前四个 |
| **抓取时问活体** | 注册表里**永远没有它们的数**，渲染那一刻现问运行时图 / 证书解析器 / ACME 管理器 | 上表后五个 |
...
新场景 [`tests/metrics/run.sh`](../../tests/metrics/run.sh)（端口段 9920–9921，
单独跑是 `METRICS_ONLY=1`），判据分节走 `[0/8]`–`[8/8]`。
