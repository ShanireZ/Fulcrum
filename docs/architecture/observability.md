...
| 面 | 形态 |
|---|---|
| **指标** | Prometheus 端点。✅ **已落地（批 M，G116–G118/G121）** —— **九个族**，覆盖请求、上游、缓存、证书与 ACME 签发，见下面「指标」一节。⚠ **连接**与 **TLS** 两类一个族都没有，已登记为 [`PLAN.md`](../../PLAN.md) §11 的 **D32** —— ★ 那不是漏做：G116 拍板的清单里本来就没有这两类 |
...
**九个族**，声明在 [`crates/fulcrum-server/src/metrics.rs`](../../crates/fulcrum-server/src/metrics.rs)
...
| 判据 | 守的是什么 |
|---|---|
| 格式 | 九个族的 HELP/TYPE 一个不少 · `Content-Type` 逐字 · `fulcrum_requests_total` 的标签键集合**逐字相等**（多一个 `uri` 就红）· 上游两族**族在、样本无**（分得开「没接上」与「没数据」）|
