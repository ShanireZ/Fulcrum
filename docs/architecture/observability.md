...
sources:
  ...
  - id: plan-11
    resource: /references/plan.md
    title: PLAN.md §11 D9（版本与兼容性策略）、D30（status_class 的第 6 个值）、D31（缓存事件的折叠口径与边界）
---
...
| 面 | 形态 |
|---|---|
| **指标** | Prometheus 端点。✅ **已落地（批 M，G116–G118/G121）** —— **九个族**，覆盖请求、上游、缓存、证书与 ACME 签发，见下面「指标」一节。⚠ G16 原话里还列了**连接**与 **TLS** 两类，今天**没有对应的族** |
