...
| **D33** | 数据面**没有线程数旋钮** ⇒ 对拍今天只能把四家全钉成 1 个 worker | M3 出数前 | 它走 pingora `ServerConf::default()`（`threads: 1`），DSL 与 CLI 里一处都没有（2026-09-05 实测）。**G35** 拍的是「线程数按 service 角色定默认、**配置可覆盖**」，后半句至今没有落地面。⛔ 别读成「枢衡只能单线程」，读作「今天没有旋钮」 → [对拍](../../bench/README.md) |
