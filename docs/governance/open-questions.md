...
| **D32** | **「连接」与「TLS」两类指标一个族都没有** | M3 | 九个族覆盖请求 · 上游 · 缓存 · 证书 · ACME 签发，连接类与 TLS 类一条没有。⚠ **出处要摆正**：G16 原话没有枚举过类别，那句「覆盖连接…TLS…」是本页从前对 G16 的**无主转述**，而 G116 拍板的清单里本来就没有这两类 ⇒ **不是实现漏做**。两条读法：那句转述不作数（代价：以后没人再想起这两类）／真欠账去补（⚠ 代价：TLS 那类想按 `sni` 分而 `sni` 由请求方给 —— G118 刚为 `host` 挡过；连接类要在 L4 与 h1/h2/h3 三条入口各取一次数 —— 那正是 D18/G66 要躲的多处取数） → [观测](/architecture/observability.md) |
