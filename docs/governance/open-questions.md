...
| **D34** | 判据 ④B（收敛）会被一个慢的被测掩盖 | M3 出第二类读数之前 | ★★★ 2026-09-06 第一趟真实数据暴露：B 取**全场极差**（那趟 0.8599，不作废），而**最快两家只差 0.0071** —— 正是它声称要怀疑的形状，它却一声没吭。⚠ 与它成对的 `BENCH_GEN_HEADROOM=0.90` **一次都没被执行过**。三条候选见 `PLAN.md` §11 |
