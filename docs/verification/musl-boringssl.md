...
## 6. 由它长出来的两条（挂号 **D21** / **D22**）—— ✅ 两条都已结案（G131 / G108）

1. ~~**D21 · 构建宿主的口径。**~~ ✅ **2026-09-04 由 G131 结案：取（a）。**
   ⛔ 决定性理由只有一条 —— （b）的全部代价落在**供应链**上；⚠ 代价认下：aarch64 那一趟
   在 qemu 上慢一个量级。✅ **那个耗时后来量到了**（曾是 D24 的输入）：2026-09-05
   `ShanireHomePC` 冷建 **18m07s**、2026-09-06 `ShanirePCX` 冷建 **34m05s**
   （⚠ 近两倍，**机器本地**，⛔ 别互相照抄）。D24 已由 **G144** 结案 = 候选 ②。
   下面这段是当初摆在面前的两条，⛔ 有意不删 —— 「当初为什么那样定」本身仍是有效信息：
   现在这份探针用的是**第二张被钉死的基础镜像**（Alpine），
