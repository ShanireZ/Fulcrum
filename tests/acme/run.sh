...
#
#   ★ 这一条**已经用上了，而且当场把根因指了出来**：上一个场景漏了个进程攥着 `:80`，
#   pingora 又是**攥着全局 `ListenFds` 锁**去做那个重试 30 秒的 bind
#   ⇒ 后面的 `:8083` 跟着起不来，而日志里只有 `:80` 的错（见 lib.sh 那段）。
#   ⛔ 仍然不许把它变成断言：源头在**别的场景没把端口还回去**，
#   在这里判只会指向一个受害者，而真凶那边一个字都不会说。
