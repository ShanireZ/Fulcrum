...
if [ "$HAS_CONTENT" = 0 ]; then
  echo "[pre-push] 这一次只有删除、没有代码往外送 —— 跳过编译门。"
  exit 0
fi
...
echo '[pre-push] 只编译、不跑测试（cargo test --no-run --workspace --all-targets）——'
echo '           判据是一句话：**编译不过的树不许离开这台机器**。'
...
if COMPILE_ONLY=1 bash "$REPO/tests/m0/docker-run.sh"; then
  echo "[pre-push] ✓ 编译通过（含全部测试目标）—— 放行。"
  exit 0
fi

echo "" >&2
echo "⛔ **push 已中止**：这棵树编译不过，或者这道门根本没能跑起来。" >&2
