...
### ★ ★ ★ 14. 加了两格数据：`SslDigest` 的 `sni` / `alpn`（2026-08-27，§10 第 79 轮，D27 + D28 结案）
...
| `prometheus` | `0.14` | **这条依赖不在了**（零命中）| ⚠ ⚠ ★ main 把它从 `pingora-core` **整条删了**。⇒ 上游下次发版时：本文件 §1 那条 `prometheus` 上界（**RUSTSEC-2024-0437** 的修法）会随依赖一起消失，而**上游自带的 `prometheus_http_app` 也会消失**。★ 我们的产品代码今天**一行都不碰它**（`crates/` 下 `prometheus` 零命中），所以不挡路；⏳ 但**观测三件套里「指标」那一件如果打算靠上游那个 app，那条路是有保质期的** |
