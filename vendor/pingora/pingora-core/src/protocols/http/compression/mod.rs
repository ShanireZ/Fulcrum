...
fn parse_accept_encoding(accept_encoding: Option<&http::HeaderValue>, list: &mut Vec<Algorithm>) {
    ...
    if let Some(ac) = accept_encoding {
        ...
        // sfv 0.15: Parser is constructed first, and parse_list() is generic over its output.
        // Tokens are now &TokenRef rather than &str, so go through as_str().
        match sfv::Parser::new(ac.as_bytes()).parse_list::<sfv::List>() {
            Ok(parsed) => {
                for item in parsed {
