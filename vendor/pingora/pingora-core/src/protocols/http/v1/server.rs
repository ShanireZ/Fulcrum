    ...
    pub async fn write_response_header(&mut self, mut header: Box<ResponseHeader>) -> Result<()> {
        ...
        match self.underlying_stream.write_all(&write_buf).await {
            Ok(()) => {
                ...
                // ── ★ ★ ★ 枢衡改动 13（2026-08-27，缺陷修复）────────────────
                //
                // 上游这里写的是 `self.body_bytes_sent += write_buf.len();` ——
                // `write_buf` 是**序列化后的响应头**，而这个计数器的名字与它自己
                // 上面那句文档都说的是 “response **body** bytes (application, not wire)”。
                //
                // ⚠ ⚠ **h2 与它不一致**：`v2/server.rs` 的 `body_sent` 只在写体那一处 `+=`
                //   （而且上游自己的三条测试断言 16 字节体 ⇒ `body_bytes_sent() == 16`）。
                //   ⇒ 同一个请求走 h1 与走 h2，这个数**差一个响应头的长度**，
                //   而没有任何东西说出这件事。
                //
                // ★ 枢衡的访问日志有一格 `resp_size`，契约写的是「响应**体**字节数（不含头）」
                //   —— 那一格是**公开契约**，不能按协议给出两个不同的答案。
                // ⇒ 这一行删掉。⚠ 全库只有 `ServerSession::body_bytes_sent()` 一个读者。
                Ok(())
            }
...
\r\n";
    ...
    /// ★ ★ ★ 枢衡改动 13 的**回归守卫**（2026-08-27）。
    ///
    /// `body_bytes_sent` 必须**只数体**，与 `v2/server.rs` 那三条断言 16 的测试同一口径。
    /// ⚠ 上游原本在 `write_response_header` 里把序列化后的响应头也加了进去
    /// ⇒ 同一个请求走 h1 与 h2 会给出两个不同的数，而枢衡的访问日志有一格
    /// `resp_size` 是**公开契约**，不能按协议给出两个答案。
    ///
    /// ★ ★ 这条测试放在**上游自己的测试模块里**，而不是放在枢衡那一侧 ——
    /// 那样它才会在 `tests/vendor/run.sh`（fork 回归网）里跑，
    /// **于是一次把那行加回来的 rebase 会当场变红**。
    #[tokio::test]
    async fn 枢衡改动13_body_bytes_sent_只数体不数头() {
        init_log();
        let wire_req = b"GET /test HTTP/1.1\r\n\r\n";
        // ⚠ ⚠ **这个 `.write(...)` 期望不是样板**：第一版没写它，于是这条测试
        //   在**干净树上也红**，而且红在 `tokio-test` 的 “unexpected write” 上 ——
        //   一条红得与它声称要守的那件事**毫无关系**的判据。
        //   ★ ★ ★ 逮到它的不是任何一道门，是**注入反证时那条失败消息读起来不对**：
        //   注入的是「把那一行加回来」，而它红的是 mock IO 没有写入期望。
        let wire_header = b"HTTP/1.1 200 OK\r\n\r\n";
        let mock_io = Builder::new()
            .read(&wire_req[..])
            .write(wire_header)
        ...
        // ⚠ 判据取 **0**，而不是「比某个数小」：一个更宽的断言在头长度变化时
        //   会跟着一起放松，而这条要守的恰恰是「头一个字节都不算进来」。
        assert_eq!(
            http_stream.body_bytes_sent(),
            0,
            "写完响应头之后，body_bytes_sent 必须还是 0 —— 头不是体"
        );
    }
}
