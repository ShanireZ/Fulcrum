...
impl RawStreamWrapper {
    pub fn new(stream: RawStream) -> Self {
        RawStreamWrapper {
            ...
            enable_rx_ts: false,
            #[cfg(target_os = "linux")]
            // ★ nix 0.31 迁移的一部分，照上游 main 改（编译器不会逼你改这两处）：
            //   ① 收的是 SO_TIMESTAMPING 的 ScmTimestampsns，内核给的是 Timestamps
            //      （system / hw_trans / hw_raw 三个 TimeSpec），按一个 TimeSpec 留空间必然截断。
            //   ② 截断在 nix 0.24 下是静默的，0.31 的 cmsgs() 会返回 ENOBUFS——
            //      而下面那个 `?` 会把它一路抛出 poll_read，于是 read 直接失败。
        ...
        match &mut rs_wrapper.stream {
            RawStream::Tcp(s) => {
                loop {
                    ...
                    // ★ 同上，照上游 main：nix 0.31 的 cmsg_space! 返回的是 **len = n 的零向量**
                    //   （0.24 是只有 capacity、len 为 0），而 recvmsg 按 len 取控制缓冲。
                    //   在新语义下 clear() 会把缓冲清成 0 字节，等于每次都截断。
